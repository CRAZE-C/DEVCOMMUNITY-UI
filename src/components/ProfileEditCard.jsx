...
export const ProfileEdit = ({ user }) => {
    ...
    const handleChangePassword = async () => {
        try {
            setError("");
            await axios.patch(`${BASE_URL}/profile/passwordUpdate`, {
                    ...
                    <div className="card-body px-2 py-2 overflow-y-auto">
                        ...
                        <div className="card-body w-full items-center gap-4">
                            ...
                            {togglePass &&
                                <>
                                    <fieldset className="w-full">
                                        <legend className="mb-2 text-sm font-medium">Old Password:</legend>
                                        <input
                                            type="text"
                                            value={password}
                                            className="w-full px-3 min-h-8 border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                                    ...
                                    <fieldset className="w-full">
                                        <legend className="mb-2 text-sm font-medium">New Password:</legend>
                                        <input
                                            type="text"
                                            value={newPassword}
                                            className="w-full px-3 min-h-8 border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                            ...
                            <button
                                className="w-full text-center text-sm text-blue-500 font-medium py-2 rounded-md hover:bg-gray-200 transition-all duration-300 ease-in-out transform hover:scale-105"
                                onClick={() => setTogglePass((value) => !value)}
                            >
                                {togglePass ? "Edit Profile" : "Change Password"}
                            </button>

                            <p className="text-red-500 w-[380px] max-w-full line-clamp-3">{error}</p>

                            <div className="card-actions pt-2 w-30">
                                <button className="btn btn-wide" onClick={togglePass ? handleChangePassword : handleSave}>
                                    {togglePass ? "Save Password" : "Save Profile"}
                                </button>
                            </div>
